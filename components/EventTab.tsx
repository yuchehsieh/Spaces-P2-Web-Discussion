        ...
        {activeSubNav === 'list' && viewingEventId && activeEvent && (
          <div className="max-w-[1500px] mx-auto animate-in fade-in slide-in-from-right-4 duration-500 flex flex-col h-full overflow-hidden">
             <div className="flex items-center gap-4 mb-10 pb-6 border-b border-slate-800">
                ...
                <div>
                   <h1 className="text-3xl font-black text-white tracking-tighter">全景連動回放 <span className="text-blue-600">.</span></h1>
             ...
             <div className="flex flex-1 gap-8 min-h-0">
                {/* Sidebar Section */}
                <div className="w-80 space-y-8 overflow-y-auto custom-scrollbar pr-2">
                   {/* 數位存證證書 Card */}
                   ...
                   <div className="space-y-4">
                      <div className="flex items-center gap-3 text-slate-400">
                         ...
                         <h4 className="text-[10px] font-black uppercase tracking-widest">回放細節</h4>
                      </div>
                      <div className="space-y-3 px-1">
                         <div className="flex justify-between items-center text-[11px] font-black uppercase tracking-widest border-b border-slate-800/50 pb-2">
                            <span className="text-slate-600">設備標籤</span>
                            <span className="text-slate-300">{activeEvent.sensorId || 'C-IPC-2'}</span>
                         </div>
                         <div className="flex justify-between items-center text-[11px] font-black uppercase tracking-widest border-b border-slate-800/50 pb-2">
                            <span className="text-slate-600">解析度</span>
                            <span className="text-slate-300 font-mono">1920x1080</span>
                         </div>
                         <div className="flex justify-between items-center text-[11px] font-black uppercase tracking-widest border-b border-slate-800/50 pb-2">
                            <span className="text-slate-600">幀率</span>
                            <span className="text-slate-300 font-mono">60 FPS</span>
                         </div>
                         <div className="flex justify-between items-center text-[11px] font-black uppercase tracking-widest pb-2">
                            <span className="text-slate-600">回放長度</span>
                ...
                {/* Main Player Section */}
                <div className="flex-1 flex flex-col bg-[#0b1121] border border-slate-800 rounded-[2.5rem] overflow-hidden shadow-2xl relative group">
                   {/* OSD - TOP */}
             ...
             <div className="mt-8 flex items-center justify-between p-8 bg-[#111827] border border-slate-800 rounded-[2.5rem] shadow-2xl relative overflow-hidden">
                ...
                <div className="flex items-center gap-10 relative z-10">
                   <div className="flex items-center gap-3">
                      ...
                      <div className="flex flex-col">
                         <span className="text-[10px] font-black text-slate-600 uppercase tracking-widest">Bitrate</span>
                         <span className="text-xs font-black text-green-500 uppercase tracking-widest">8.4 Mbps</span>
                      </div>
                   </div>
                   <div className="flex items-center gap-3">
                      ...
                      <div className="flex flex-col">
                         <span className="text-[10px] font-black text-slate-600 uppercase tracking-widest">Storage</span>
                         <span className="text-xs font-black text-blue-400 uppercase tracking-widest">SK-DATA-SEC-B</span>
                      </div>
                   </div>
                </div>

                <div className="flex gap-4 relative z-10">
                   <button className="px-8 py-3.5 bg-slate-800 hover:bg-slate-700 text-white rounded-2xl transition-all font-black text-xs uppercase tracking-[0.2em] border border-slate-700 flex items-center justify-center gap-3 shadow-xl">
                      <Download size={18} className="text-blue-400" /> 下載數位存證
                   </button>
                   <button 
                     onClick={() => setViewingEventId(null)}
                     className="px-12 py-3.5 bg-blue-600 hover:bg-blue-500 text-white rounded-2xl transition-all font-black text-xs uppercase tracking-[0.2em] shadow-2xl shadow-blue-900/30"
                   >
                      確認並關閉
                   </button>
                </div>
        ...
        {activeSubNav === 'settings' && isCreating && (
          <div className="max-w-[1400px] mx-auto animate-in fade-in slide-in-from-right-4 duration-500">
             ...
             <div className="mb-10">
                ...
                <input 
                   type="text" 
                   placeholder="EVEVENT NAME"
                   value={newEventName}
                   onChange={(e) => setNewEventName(e.target.value)}
                   className="w-full max-w-2xl bg-[#111827] border border-slate-700 rounded-2xl py-4 px-6 text-xl font-black text-white focus:outline-none focus:border-blue-500 transition-all placeholder:text-slate-800"
                />
             </div>
                ...
                <div className="bg-[#111827] border border-slate-800 rounded-[2.5rem] p-8 shadow-xl flex flex-col">
                   ...
                   <div className="space-y-4">
                      ...
                      <div className="space-y-3 overflow-y-auto max-h-[450px] pr-2 custom-scrollbar">
                         {selectedAssetCategory === 'sensor' && (
                           <>
                              <UnifiedScopeItem label="Environmental Sensors" icon={<Activity size={16}/>} sub="Temp, Humidity" defaultChecked />
                              <UnifiedScopeItem label="Water Leak Sensors" icon={<Shield size={16}/>} sub="Flooding Detection" defaultChecked />
                         ...
                         {selectedAssetCategory === 'camera' && (
                           <>
                              <UnifiedScopeItem label="Entrance Camera 01" icon={<Video size={16}/>} sub="HQ-Zone-A" defaultChecked />
                              <UnifiedScopeItem label="Backdoor Camera 04" icon={<Video size={16}/>} sub="HQ-Zone-B" />
                              <UnifiedScopeItem label="Warehouse Camera 02" icon={<Video size={16}/>} sub="ZS-Zone-C" defaultChecked />
                           </>
                         )}
                         {selectedAssetCategory === 'energy' && (
                           <>
                              <UnifiedScopeItem label="Smart Meter - Main" icon={<Zap size={16}/>} sub="Total Consumption" defaultChecked />
                              <UnifiedScopeItem label="Smart Meter - Server Rm" icon={<Zap size={16}/>} sub="Data Center Feed" />
                           </>
                         )}
                      </div>
                   </div>
                           ...
                           <div className="space-y-6 animate-in fade-in slide-in-from-top-2 duration-300">
                              ...
                              <div className="space-y-4">
                                 ...
                                 <div className="relative mb-4">
                                    <input type="text" placeholder="Search user..." className="w-full bg-[#050914] border border-slate-800 rounded-xl py-2.5 pl-10 pr-4 text-xs text-slate-300 focus:outline-none focus:border-blue-500" />
                                    <Search size={14} className="absolute left-3.5 top-3 text-slate-700" />
                                 </div>
                                 <div className="space-y-2 overflow-y-auto max-h-[180px] pr-2 custom-scrollbar">
                                    <UnifiedCheckbox label="Admin (admin@sks.com.tw)" checked={true} onChange={() => {}} small />
                                    <UnifiedCheckbox label="Bob (bob@sks.com.tw)" checked={true} onChange={() => {}} small />
                                    <UnifiedCheckbox label="Mary (mary@sks.com.tw)" checked={false} onChange={() => {}} small />
                                 </div>
                              </div>
                           </div>
                         )}
                         {selectedActions.includes('webhook') && (
                           <div className="space-y-3 pt-6 border-t border-slate-800 animate-in fade-in slide-in-from-top-2 duration-300">
                              ...
                              <div className="space-y-3">
                                ...
                                <textarea placeholder="PAYLOAD HEADERS (JSON)" className="w-full bg-[#050914] border border-slate-800 rounded-xl py-3 px-4 text-xs text-slate-300 focus:outline-none focus:border-blue-500 h-16 resize-none font-mono" />
                              </div>
                           </div>
                         )}
                      </div>
...
const UnifiedCheckbox: React.FC<{ label: string; desc?: string; checked?: boolean; onChange: () => void; small?: boolean }> = ({ label, desc, checked, onChange, small }) => (
  <label onClick={onChange} className={`flex items-center gap-4 ${small ? 'p-2' : 'p-4'} bg-slate-900/40 rounded-2xl border border-slate-800 hover:border-slate-600 cursor-pointer transition-all group`}>
     ...
     <div className="flex flex-col">
        <span className={`${small ? 'text-xs' : 'text-sm'} font-bold ${checked ? 'text-white' : 'text-slate-400 group-hover:text-slate-300'}`}>{label}</span>
        {desc && <span className="text-[9px] text-slate-500 font-medium">{desc}</span>}
     </div>
  </label>
);

const UnifiedScopeItem: React.FC<{ label: string; sub: string; icon: React.ReactNode; defaultChecked?: boolean; disabled?: boolean }> = ({ label, sub, icon, defaultChecked, disabled }) => (
  <label className={`flex items-start gap-4 p-4 rounded-2xl border transition-all ${disabled ? 'bg-[#03060c] border-[#0c121e] opacity-20 cursor-not-allowed shadow-none' : 'bg-slate-900/40 border-slate-800 cursor-pointer hover:border-slate-600 hover:bg-slate-800/50 shadow-md'}`}>
     <div className="mt-1 relative">
        <div className={`w-5 h-5 border-2 rounded flex items-center justify-center transition-all ${disabled ? 'bg-[#010204] border-[#050a14]' : (defaultChecked ? 'bg-blue-600 border-blue-500' : 'bg-slate-900 border-slate-700')}`}>
           {defaultChecked && !disabled && <Check size={12} className="text-white" />}
        </div>
     </div>
     <div className="flex flex-col min-w-0">
        <div className="flex items-center gap-2 mb-0.5">
           <span className={`${disabled ? 'text-[#050a14]' : 'text-slate-400'}`}>{icon}</span>
           <span className={`text-xs font-bold truncate ${disabled ? 'text-[#050a14]' : 'text-slate-200'}`}>{label}</span>
        </div>
        <span className={`text-[9px] font-medium ${disabled ? 'text-[#020408]' : 'text-slate-500'}`}>{sub}</span>
     </div>
  </label>
);
