...
const TreeItem: React.FC<TreeItemProps> = ({ 
  ...
  // 反灰邏輯：根據需求，只有「設備」且「已配置」才反灰
  const isPlaced = isDevice && placedDeviceIds.has(node.id);
  
  // 檢查是否為「目前選中節點的直接子節點」 (用於 GIS 配置判斷)
  ...
  const canDrag = isEditing && (
    (isDevice && !isPlaced && parentId === selectedId && selectedNodeType === 'zone') ||
...
const FloorPlanCenterTab: React.FC<{ initialSiteId?: string | null }> = ({ initialSiteId }) => {
  ...
  const createMapRegion = (map: any, regionConfig: MapRegion, label: string = '區域範圍', isViewMode: boolean = false) => {
    ...
    const polygon = L.polygon(coords, { 
      ...
      weight: 3, 
      fillColor: '#3b82f6', 
      fillOpacity: isViewMode ? 0.2 : 0.1, 
      dashArray: isViewMode ? null : '8, 8' 
    }).addTo(map);
    ...
    (window as any).deleteMapPin = (id: string) => { 
        setFloorPlans(prev => {
            ...
            if (i > -1 && next[i].mapConfig) {
                next[i].mapConfig = {
                    ...next[i].mapConfig!,
                    pins: next[i].mapConfig?.pins?.filter(p => p.id !== id) || []
                };
            }
            return next;
        });
    ...
    (window as any).deleteMapRegion = (id: string) => { 
        setFloorPlans(prev => {
            ...
            if (i > -1 && next[i].mapConfig) {
                next[i].mapConfig = {
                    ...next[i].mapConfig!,
                    regions: next[i].mapConfig?.regions?.filter(r => r.id !== id) || []
                };
            }
            return next;
        });
    ...
    if (isEditing && sourceType === 'map' && mapContainerRef.current) {
      ...
      const timer = setTimeout(() => {
        ...
        if (activeFloorPlan?.mapConfig?.regions) {
            activeFloorPlan.mapConfig.regions.forEach(r => {
                const node = findNodeById(SITE_TREE_DATA, r.id);
                createMapRegion(map, r, node?.label);
            });
        }
        if (activeFloorPlan?.mapConfig?.pins) {
            activeFloorPlan.mapConfig.pins.forEach(p => createMapPin(map, p));
        }
        if (activeFloorPlan?.sensors) {
            activeFloorPlan.sensors.forEach(s => {
                const n = findNodeById(SITE_TREE_DATA, s.id);
                createDeviceMarker(map, s, n?.label || '未知設備', true);
            });
        }
  ...
  useEffect(() => {
    if (!isEditing && sourceType === 'map' && viewMapContainerRef.current) {
      ...
      const timer = setTimeout(() => {
        ...
        // 渲染 Regions (檢視模式帶導航)
        activeFloorPlan?.mapConfig?.regions?.forEach((r:any) => {
          const node = findNodeById(SITE_TREE_DATA, r.id);
          createMapRegion(map, r, node?.label || '', true);
        });

        // 渲染 Pins (檢視模式帶導航)
        activeFloorPlan?.mapConfig?.pins?.forEach((p:any) => {
          createMapPin(map, p, true);
        });

        // 渲染設備 (檢視模式)
        activeFloorPlan?.sensors.forEach(s => { 
          const n = findNodeById(SITE_TREE_DATA, s.id); 
          createDeviceMarker(map, s, n?.label || '', false); 
        });
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    if (sourceType === 'map' && mapRef.current) {
      ...
      // 1. 處理子節點拖拽 (據點/區域入口)
      if (nodeId && nodeType !== 'device') {
        setPendingPlacement({ id: nodeId, label: nodeLabel, type: nodeType, lat: latlng.lat, lng: latlng.lng });
        return;
      }
      ...
      if (sensorId && selectedNode?.type === 'zone') {
        ...
        setFloorPlans(prev => { 
          ...
          const idx = next.findIndex(p => p.siteId === selectedSiteId); 
          if (idx === -1) {
            next.push({ siteId: selectedSiteId, type: 'map', sensors: [newSensor], mapConfig: { center: [mapRef.current.getCenter().lat, mapRef.current.getCenter().lng], zoom: mapRef.current.getZoom(), regions: [], pins: [] } }); 
          } else { 
            const ns = [...next[idx].sensors]; 
    ...
    } else if (sourceType === 'image' && imgRef.current && selectedNode?.type === 'zone') {
      const rect = imgRef.current.getBoundingClientRect();
  ...
  const handlePlacementChoice = (choice: 'pin' | 'region') => {
    if (!pendingPlacement || !mapRef.current) return;
    const { id, label, lat, lng } = pendingPlacement;

    if (choice === 'pin') {
      const newPin = { id, label, lat, lng };
      createMapPin(mapRef.current, newPin);
      setFloorPlans(prev => {
        const next = [...prev];
        const idx = next.findIndex(p => p.siteId === selectedSiteId);
        if (idx === -1) {
            next.push({ siteId: selectedSiteId, type: 'map', sensors: [], mapConfig: { center: [lat, lng], zoom: 17, regions: [], pins: [newPin] } });
        } else {
            const mc = next[idx].mapConfig || { center: [lat, lng], zoom: 17, regions: [], pins: [] };
      ...
      const offset = 0.0004;
      const newRegion = { id, coords: [[lat + offset, lng - offset], [lat + offset, lng + offset], [lat - offset, lng + offset], [lat - offset, lng - offset]] as [number, number][] };
      createMapRegion(mapRef.current, newRegion, label);
      setFloorPlans(prev => {
        const next = [...prev];
        const idx = next.findIndex(p => p.siteId === selectedSiteId);
        if (idx === -1) {
            next.push({ siteId: selectedSiteId, type: 'map', sensors: [], mapConfig: { center: [lat, lng], zoom: 17, regions: [newRegion], pins: [] } });
        } else {
            const mc = next[idx].mapConfig || { center: [lat, lng], zoom: 17, regions: [], pins: [] };
  ...
  return (
    <div className="flex h-full w-full bg-[#050914] text-slate-200 overflow-hidden relative font-sans">
      ...
      <div className="w-80 border-r border-slate-800 bg-[#0b1121] flex flex-col shrink-0 overflow-hidden relative z-50">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-3">
          {SITE_TREE_DATA.map(node => (
            <TreeItem 
              ...
              onSelect={(id) => { setSelectedSiteId(id); setIsEditing(false); }} 
              searchTerm={searchTerm} 
              idsWithFloorPlan={idsWithFloorPlan} 
              placedDeviceIds={placedDeviceIds}
              representedSubNodeIds={representedSubNodeIds}
        ...
        <div className="p-3 border-t border-slate-800/50 bg-black/20">
           <div className="text-[9px] font-black text-slate-600 uppercase tracking-[0.2em] text-center italic">
              {isEditing ? 'DRAG SUB-SITES OR DEVICES TO GIS' : 'SITE HIERARCHY'}
           </div>
        </div>
      </div>
            ...
            <div className="p-4 bg-[#111827] border-b border-slate-800 flex items-center justify-between z-[100] shrink-0">
              ...
              <div className="flex items-center gap-3">
                {selectedNode?.type !== 'device' && (
                    ...
                    <>
                        {!!activeFloorPlan && (
                        <button onClick={() => setIsResetting(true)} className="px-5 py-2.5 bg-red-950/20 hover:bg-red-600/20 text-red-500 rounded-xl font-bold text-xs border border-red-900/30 flex items-center gap-2 transition-all shadow-lg"><RefreshCw size={14}/> 重新配置圖資</button>
                        )}
            ...
            >
                ...
                {selectedNode?.type === 'device' ? (
                   <div className="flex flex-col items-center justify-center h-full gap-8 animate-in zoom-in-95 duration-500">
                        <div className="w-48 h-48 rounded-[3.5rem] bg-slate-900/50 border border-slate-800 flex items-center justify-center text-slate-700 shadow-2xl">
                    ...
                    <>
                        ...
                        {isEditing && sourceType === 'image' && !activeFloorPlan?.imageUrl && (
                          <div className="flex flex-col items-center gap-8 animate-in zoom-in-95">
                              ...
                              <div 
                                ...
                                onDragOver={(e) => e.preventDefault()}
                                onDrop={(e) => { e.preventDefault(); const file = e.dataTransfer.files?.[0]; if (file) { const ev = { target: { files: [file] } } as any; handleFileSelect(ev); } }}
                                className="w-[32rem] h-80 bg-[#111827] border-4 border-dashed border-slate-800 rounded-[3.5rem] hover:border-blue-500 hover:bg-blue-600/5 transition-all group cursor-pointer flex flex-col items-center justify-center gap-6 shadow-2xl relative overflow-hidden"
                              >
                                  <div className="p-8 bg-slate-800 rounded-3xl text-slate-400 group-hover:text-blue-400 transition-all shadow-inner group-hover:scale-110">
                                      <Upload size={48} />
                                  </div>
                                  <div className="text-center space-y-2">
                                      <span className="text-xl font-black text-white uppercase tracking-widest block">點擊或拖曳檔案至此</span>
                                      <p className="text-xs text-slate-500 font-bold uppercase tracking-widest">支援 JPG, PNG, BMP 格式 (建議 1920x1080 以上)</p>
                                  </div>
                        ...
                        <div className="w-full h-full relative">
                                            ...
                                            <div className="max-h-60 overflow-y-auto custom-scrollbar">
                                                {addressSuggestions.map((s, idx) => (
                                                    <button key={idx} onClick={() => { setAddressSearch(s.address); performAddressSearch(s.address); }} className="w-full text-left p-4 hover:bg-blue-600/10 border-b border-slate-800/50 last:border-none transition-colors group flex items-start gap-4">
                                                        <div className="p-2 bg-slate-800 rounded-lg text-slate-400 group-hover:text-blue-400 group-hover:bg-blue-600/10 transition-all"><MapPin size={16} /></div>
                                ...
                                <div className="absolute inset-0 z-[700] bg-black/40 backdrop-blur-[2px] flex items-center justify-center animate-in fade-in duration-200">
                                    <div className="bg-[#1e293b] border border-slate-700 rounded-[2.5rem] p-8 shadow-2xl animate-in zoom-in-95 w-full max-w-sm flex flex-col gap-6 ring-1 ring-white/5">
                                        <div className="text-center">
                                            <span className="text-[10px] font-black text-blue-400 uppercase tracking-[0.2em] block mb-2">GIS PLACEMENT MODE</span>
                                            <h3 className="text-xl font-black text-white italic tracking-tighter truncate">「{pendingPlacement.label}」</h3>
                                            <p className="text-xs text-slate-500 font-bold mt-2 leading-relaxed px-4">請選取要在目前 GIS 圖資上標註的形式，此標註將作為下層區域之入口點。</p>
                                        </div>
                                        <div className="grid grid-cols-2 gap-4">
                                            <button 
                                                onClick={() => handlePlacementChoice('pin')}
                                                className="flex flex-col items-center justify-center gap-4 p-6 bg-slate-900 border border-slate-800 rounded-3xl hover:border-red-500 hover:bg-red-500/10 transition-all group"
                                            >
                                                <div className="p-3 bg-red-600 text-white rounded-xl shadow-lg shadow-red-900/40 group-hover:scale-110 transition-transform"><MapPinIcon size={24}/></div>
                                                <span className="text-[11px] font-black uppercase tracking-widest text-slate-400 group-hover:text-red-400">據點標註</span>
                                            </button>
                                            <button 
                                                onClick={() => handlePlacementChoice('region')}
                                                className="flex flex-col items-center justify-center gap-4 p-6 bg-slate-900 border border-slate-800 rounded-3xl hover:border-blue-500 hover:bg-blue-500/10 transition-all group"
                                            >
                                                <div className="p-3 bg-blue-600 text-white rounded-xl shadow-lg shadow-blue-900/40 group-hover:scale-110 transition-transform"><Square size={24}/></div>
                                                <span className="text-[11px] font-black uppercase tracking-widest text-slate-400 group-hover:text-red-400">區域範圍</span>
                                            </button>
                                        </div>
                            ...
                            {isEditing && (
                                <button onClick={() => setIsMapSettingsOpen(true)} className="p-4 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-2xl shadow-xl transition-all border border-slate-700"><Settings size={24} /></button>
                            )}
           ...
           <div className="bg-[#1e293b] border border-slate-700 rounded-[2.5rem] p-10 max-w-sm w-full animate-in zoom-in shadow-2xl ring-1 ring-white/5">
              ...
              <div className="flex gap-4">
                 ...
                 <button onClick={() => { 
                    setFloorPlans(p => { 
                        const next = [...p]; 
                        const i = next.findIndex(x => x.siteId === selectedSiteId); 
                        if (i > -1) {
                            next[i] = {
                                ...next[i],
                                sensors: next[i].sensors.filter(s => s.id !== deviceToDelete.id)
                            };
                        }
                        return next; 
                    }); 
                    setDeviceToDelete(null); 
                 }} className="flex-1 py-4 bg-red-600 hover:bg-red-500 text-white rounded-2xl font-black uppercase text-xs shadow-xl shadow-red-900/20 active:scale-95 transition-all">確認移除</button>
              </div>
           </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[1200] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in">
           <div className="bg-[#1e293b] border border-slate-700 rounded-[2.5rem] p-10 max-w-sm w-full animate-in zoom-in shadow-2xl ring-1 ring-white/5">
              ...
              <div className="flex gap-4">
                 ...
                 <button onClick={() => { setFloorPlans(p => p.filter(x => x.siteId !== selectedSiteId)); setSourceType(null); setIsResetting(false); setIsEditing(true); }} className="flex-1 py-4 bg-red-600 hover:bg-red-500 text-white rounded-2xl font-black uppercase text-xs shadow-xl shadow-red-900/20 active:scale-95 transition-all">確認重新配置</button>
              </div>
           </div>
        </div>
      )}
